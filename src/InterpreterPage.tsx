...
export default function InterpreterPage() {
  ...
  return (
    ...
    <div className="min-h-screen bg-background text-foreground font-sans overflow-x-hidden flex flex-col custom-scrollbar">
      ...
      <main className="flex-1 pt-24 md:pt-32 pb-20 container mx-auto px-6 flex flex-col items-center">
        ...
        {/* ─── Tab switcher ─── */}
        <div className="bg-muted border border-border p-1.5 rounded-full flex gap-1 shadow-xl mb-12">
          <button
        ...
        <div className="w-full max-w-5xl grid md:grid-cols-2 gap-6">
          ...
          {/* LEFT card: Input */}
          <div className="bg-card border border-border rounded-3xl p-8 flex flex-col gap-6 backdrop-blur-sm">
            ...
            {activeTab === "sign-to-text" ? (
              /* Camera view */
              <div className="relative bg-muted rounded-2xl overflow-hidden aspect-video flex flex-col items-center justify-center border border-border">
                <video
                  ref={videoRef}
              ...
              <div className="flex flex-col gap-4 flex-1">
                <textarea
                  ...
                  className="flex-1 min-h-[180px] bg-muted border border-border rounded-2xl p-5 text-foreground placeholder:text-muted-foreground text-base font-medium resize-none focus:outline-none focus:border-emerald-500/40 transition-colors"
                />
                <button
            ...
            {activeTab === "sign-to-text" && (
              <div className="flex items-center gap-3 mt-2">
                ...
                <div className="flex bg-muted border border-border rounded-full p-1 gap-1">
                  {[["rw", "Kinyarwanda"], ["en", "English"]].map(([v, label]) => (
                    <button
                      key={v}
                      onClick={() => setOutputLang(v)}
                      className={`px-4 py-1.5 rounded-full text-xs font-bold transition-all ${outputLang === v ? "bg-emerald-500 text-white shadow-md" : "text-muted-foreground hover:text-foreground"}`}
                    >
                      {label}
                    </button>
                  ))}
          ...
          <div className="bg-card border border-border rounded-3xl p-8 flex flex-col gap-6 backdrop-blur-sm">
            <div className="flex items-center justify-between">
              <h3 className="font-bold text-base text-card-foreground">{t.rightCard}</h3>
              {cameraActive && (
            ...
            {activeTab === "sign-to-text" ? (
              <div className="flex-1 bg-muted rounded-2xl border border-border p-6 flex flex-col min-h-[240px] relative">
                {cameraActive ? (
                  <>
                    ...
                    <div className="flex items-start gap-4 mb-4">
                      ...
                      <div className="flex flex-col gap-2 pt-1">
                        <button onClick={commitLetter} className="px-4 py-2 rounded-xl bg-background text-xs font-bold text-muted-foreground hover:bg-accent transition-colors border border-border">
                          {t.addLetter}
                        </button>
                        <button onClick={commitSpace} className="px-4 py-2 rounded-xl bg-background text-xs font-bold text-muted-foreground hover:bg-accent transition-colors border border-border">
                          {t.addSpace}
                        </button>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between mt-4 pt-4 border-t border-border">
                      ...
                      <button onClick={speakResult} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-background text-xs font-bold text-muted-foreground hover:bg-emerald-500/20 hover:text-emerald-400 transition-colors border border-border">
                        <IcVolume /> {t.speak}
                      </button>
                    </div>
                  </>
              ...
              /* Text-to-Sign output */
              <div className="flex-1 bg-muted rounded-2xl border border-border p-6 flex flex-col min-h-[240px] overflow-hidden">
        ...
        {/* Logs drawer */}
        {showLogs && (
          <div className="w-full max-w-5xl mt-6 rounded-3xl bg-muted border border-border p-8 overflow-hidden">
